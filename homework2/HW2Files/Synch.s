...
_StringConst_5:
	.word	48			! length
	.ascii	"This Thread is not the lockHolder, unlock failed"
	.align
_StringConst_4:
	.word	40			! length
	.ascii	"LockHolder currently exists, lock failed"
	.align
_StringConst_3:
	.word	51			! length
...
_CheckVersion_P_Synch_:
	...
	set	0xdb3d1cb0,r4		! myHashVal = -616751952
	cmp	r3,r4
	be	_Label_9
	set	_CVMess1,r1
	call	_putString
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if lockHolder != _P_Thread_currentThread then goto _Label_56		(int)
	load	[r14+8],r1
	load	[r1+4],r1
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if lockHolder == 0 then goto _Label_61		(int)
	load	[r14+8],r1
	load	[r1+4],r1
	mov	0,r2
	cmp	r1,r2
...
! ASSIGNMENT STATEMENT...
	mov	110,r13		! source line 110
	mov	"\0\0AS",r10
!   lockHolder = _P_Thread_currentThread		(4 bytes)
	set	_P_Thread_currentThread,r1
	load	[r1],r1
	load	[r14+8],r2
	store	r1,[r2+4]
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if lockHolder == _P_Thread_currentThread then goto _Label_72		(int)
	load	[r14+8],r1
	load	[r1+4],r1
	set	_P_Thread_currentThread,r2
	load	[r2],r2
...
! RETURN STATEMENT...
	...
	mov	"\0\0RE",r10
!   if lockHolder != _P_Thread_currentThread then goto _Label_90		(int)
	load	[r14+8],r1
	load	[r1+4],r1
	set	_P_Thread_currentThread,r2
	load	[r2],r2
